...
router.get('/:event/:round/:limit?', async function(req, res, next) {
    const ev = new String(req.params.event)
    const round = Number(req.params.round)
    const limit = (req.params.limit == undefined) ? 999 : Number(req.params.limit)

    try {
        const records = await Record.find({ compId: req.compId, event: ev, round: round })
            .sort({ place: 1 })
            .limit(limit)
